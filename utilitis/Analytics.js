...
export const generateCumulativeMonthlyData = async (
  model,
  monthsBack = 6,
  cache = {}
) => {
  ...
  try {
    // Set the date for the start of the range
    const startDate = new Date();
    startDate.setMonth(startDate.getMonth() - (monthsBack - 1), 1); // Start of the first month

    // Aggregation pipeline to get counts per month
    const monthlyCounts = await model.aggregate([
      {
    ...
    // Populate results and accumulate counts
    const currentDate = new Date();
    let cumulativeCount = 0; // Store accumulated count
    ...
    for (let i = monthsBack - 1; i >= 0; i--) {
      ...
      cumulativeCount += match ? match.count : 0; // Add current month count to cumulative count
